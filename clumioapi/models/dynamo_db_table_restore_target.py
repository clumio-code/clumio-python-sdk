...
class DynamoDBTableRestoreTarget:
    ...
    standard storage class if empty.
            ...
            Tags:
                The aws tags to be applied to the restored table. the tags are stored in the aws cloud as part of your aws account.
    a dynamodb table can be have multiple tags. to find the tags that were applied to the original table,
    use the [get /datasources/aws/dynamodb-tables/{table_id}](#operation/read-aws-dynamodb-table) endpoint to display
    the original table's tag keys (`tags.key`) and tag values (`tags.value`).
    the restored table will not have any tags applied if this is specified as `null`.
