...
class BackupAwsEbsVolumesV1Controller(base_controller.BaseController):
    ...
    def create_backup_aws_ebs_volume(
        self,
        embed: str,
        body: create_backup_aws_ebs_volume_v1_request.CreateBackupAwsEbsVolumeV1Request,
        **kwargs,
    ) -> Union[
        ...
        if self.config.raw_response:
            return (
                resp,
                on_demand_ebs_backup_response_v1.OnDemandEBSBackupResponseV1.from_dictionary(
                    resp.json()
                ),
            )
