...
class VmwareVcenterComplianceStatsV1Controller(base_controller.BaseController):
    ...
    def __init__(self, config: configuration.Configuration) -> None:
        ...
        self.config = config
        self.headers = {
            'accept': 'application/api.clumio.vmware-vcenter-compliance-stats=v1+json',
    ...
    def read_vmware_vcenter_compliance_stats(self, vcenter_id: str, **kwargs) -> Union[
        ...
        # Prepare query URL
        _url_path = (
            '/datasources/vmware/vcenters/{vcenter_id}/stats/compliance'
        )
        ...
        if self.config.raw_response:
            return (
                resp,
                read_v_mware_v_center_protection_stats_response.ReadVMwareVCenterProtectionStatsResponse.from_dictionary(
                    resp.json()
                ),
            )
