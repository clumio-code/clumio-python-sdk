...
class BackupAwsDynamodbTablesV1ControllerPaginator(base_controller.BaseController):
    """A Controller to access Endpoints for backup-aws-dynamodb-tables resource with pagination."""

    def __init__(self, config: configuration.Configuration) -> None:
        super().__init__(config)
        self.controller = BackupAwsDynamodbTablesV1Controller(config)
    ...
    def list_backup_aws_dynamodb_tables(
        ...
        **kwargs,
        ...
        start = start or '1'
        while True:
            response = self.controller.list_backup_aws_dynamodb_tables(
