...
class AwsRdsResourcesV1Controller(base_controller.BaseController):
    ...
    def list_aws_rds_resources(
        ...
        **kwargs,
        ...
        def get_instance_from_response(response: requests.Response) -> Any:
            return list_rds_resources_response.ListRdsResourcesResponse.from_response(response)
        ...
        resp_instance: list_rds_resources_response.ListRdsResourcesResponse
        # Execute request
        ...
        if not resp.ok:
            error_str = f'list_aws_rds_resources for url {urllib.parse.unquote(resp.url)} failed.'
    ...
    def read_aws_rds_resource(
        ...
        **kwargs,
        ...
        def get_instance_from_response(response: requests.Response) -> Any:
            return read_rds_resource_response.ReadRdsResourceResponse.from_response(response)
        ...
        resp_instance: read_rds_resource_response.ReadRdsResourceResponse
        # Execute request
        resp: requests.Response
        try:
            resp = self.client.get(
        ...
        if not resp.ok:
            error_str = f'read_aws_rds_resource for url {urllib.parse.unquote(resp.url)} failed.'
            raise clumio_exception.ClumioException(error_str, resp=resp)
...
class AwsRdsResourcesV1ControllerPaginator(base_controller.BaseController):
    """A Controller to access Endpoints for aws-rds-resources resource with pagination."""

    def __init__(self, config: configuration.Configuration) -> None:
        super().__init__(config)
        self.controller = AwsRdsResourcesV1Controller(config)
    ...
    def list_aws_rds_resources(
        ...
        lookback_days: int | None = None,
        **kwargs,
    ) -> Iterator[list_rds_resources_response.ListRdsResourcesResponse]:
        """Retrieve a list of RDS resources.
        ...
        start = start or '1'
        while True:
            response = self.controller.list_aws_rds_resources(
                limit=limit,
                start=start,
                filter=filter,
                embed=embed,
